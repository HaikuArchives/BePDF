...
SplashFTFont::SplashFTFont(SplashFTFontFile *fontFileA, SplashCoord *matA,
			   SplashCoord *textMatA, GBool hintingA):
...
GBool SplashFTFont::makeGlyph(int c, int xFrac, int yFrac,
  ...
  if (aa) {
    if (hinting) {
    	mode = FT_LOAD_DEFAULT | FT_LOAD_NO_BITMAP;
    } else {
    	mode = FT_LOAD_NO_HINTING | FT_LOAD_NO_BITMAP;
    }
  } else {
    if (hinting) {
    	mode = FT_LOAD_DEFAULT;
    } else {
    	mode = FT_LOAD_NO_HINTING;
    }
  }
  if (FT_Load_Glyph(ff->face, gid, mode)) {
    return gFalse;
  }
#else

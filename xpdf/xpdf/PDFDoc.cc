...
PDFDoc::PDFDoc(GString *fileNameA, GString *ownerPassword,
  ...
  FILE *f = NULL;
#ifdef VMS
  if (!(f = fopen(fileName1->getCString(), "rb", "ctx=stm"))) {
    error(-1, "Couldn't open file '%s'", fileName1->getCString());
    errCode = errOpenFile;
    return;
  }
#else
  if (!(f = fopen(fileName1->getCString(), "rb"))) {
    fileName2 = fileName->copy();
    fileName2->lowerCase();
    if (!(f = fopen(fileName2->getCString(), "rb"))) {
      fileName2->upperCase();
      if (!(f = fopen(fileName2->getCString(), "rb"))) {
  ...
  ok = setup(ownerPassword, userPassword, monitor);
}
...
GBool PDFDoc::setup(GString *ownerPassword, GString *userPassword, 
  ...
  // read catalog
  catalog = new Catalog(xref, monitor);
  if (!catalog->isOk()) {
  ...
  if (monitor != NULL) {
    monitor->setStage(LoadProgressMonitor::kDoneLoadPages);  
  }

...
Catalog::Catalog(XRef *xrefA, LoadProgressMonitor *monitor) {
  ...
  if (monitor != NULL) {
    monitor->setStage(LoadProgressMonitor::kBeginLoadPages);  
    monitor->setNumPages(numPages0);  
  }
  obj.free();
  ...
  pagesDictRef.free();
  numPages = readPageTree(pagesDict.getDict(), NULL, 0, alreadyRead, monitor);
  gfree(alreadyRead);
  ...
  // read root of embedded files name tree
  if (catDict.dictLookup("Names", &obj)->isDict())
    obj.dictLookup("EmbeddedFiles", &embeddedFiles);
  else
    embeddedFiles.initNull();
  obj.free();
  // read page labels
  catDict.dictLookup("PageLabels", &pageLabels);
  // read the interactive form dictionary
  catDict.dictLookupNF("AcroForm", &acroFormRef);
#endif
...
int Catalog::readPageTree(Dict *pagesDict, PageAttrs *attrs, int start,
			  char *alreadyRead, LoadProgressMonitor *monitor) {
  Object kids;
  Object kid;
  Object kidRef;
  ...
  for (i = 0; i < kids.arrayGetLength(); ++i) {
    ...
    // PDF file where the /Type entry is missing.
      ...
      if ((start = readPageTree(kid.getDict(), attrs1, start, alreadyRead, monitor))
	  < 0)
	goto err2;
    } else {
